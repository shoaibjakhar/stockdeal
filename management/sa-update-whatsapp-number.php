...
$connect = mysqli_connect('localhost','bigsenrl_wp721','S0]7jp5(3b','bigsenrl_wp721');
if(!$connect)
{
die('Could not connect!' . mysqli_error);
}

//mysql_select_db('shareevx_wp576', $connect);

 $sql = "SELECT * FROM `wpbr_options` WHERE  option_id = '564056'";
...
if($_POST){
   ...
   $upd_qry = "UPDATE `wpbr_options` SET `option_value` = '".$new_content."' WHERE  option_id = '564056' ";
   mysqli_query($connect,$upd_qry);
  //header('location:');
  echo '<script>window.location.href="sa-update-whatsapp-number.php?t='.time().'&updated=true"</script>';
