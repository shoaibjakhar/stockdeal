...
<?php
    ...
    //global $username;
        ...
        $TL_3_total = 0;
           ...
           where  
           (Agent_1_TL='".$users."' OR Agent_2_TL='".$users."' OR Agent_3_TL='".$users."') && SaleDate BETWEEN '".$_GET['from']."' AND '".$_GET['to']."' AND Approval_Status = 'Approved' ORDER BY  `Costumer_ID` DESC");
           }
           else{
               $sql = ("SELECT SUM(Agent_1_Shared_Amount) AS Total1, SUM(Agent_2_Shared_Amount) AS Total2 , SUM(Agent_3_Shared_Amount) AS Total3 
               FROM Customer_Payment_History 
...
function getShared1($users){

    global $connect;
    if(isset($_GET['from']) && isset($_GET['to'])){
      $sql = ("SELECT id,Costumer_ID, DATE_FORMAT( SaleDate,  '%d-%m-%Y' ) AS SaleDateIND, Full_Name, LastName, Email_ID, Mobile_No, Location, PackageName,  DATE_FORMAT( Activation_Date,  '%d-%m-%Y' ) AS ActivationDate ,  DATE_FORMAT( Exp_Date,  '%d-%m-%Y' ) AS ExpDate , case when Exp_Date< NOW() then 'Expired' else 'Active' end as Status , Remark, Paid_Amout, Company_Amount, Tax_Amount, PaymentMode, Agent, Manager, DATE_FORMAT( DateTime,  '%d-%m-%Y %h %i' ) AS DateTimeConvert  FROM Customer_Payment_History where  (Agent_1_TL = '".$users."' or Agent_2_TL = '".$users."' or Agent_3_TL = '".$users."') && SaleDate BETWEEN '".$_GET['from']."' AND '".$_GET['to']."' AND Approval_Status = 'Approved' ORDER BY  `Costumer_ID` DESC");
    }
    else {
...
//   }
    ...
    $sel = "select * from employee where Status = 'Active'  AND Role ='Team Leader' ";
}
$qry = mysqli_query($connect,$sel);
    ...
    <div class="col-md-4">
        ...
        <select class="form-control" name="Team_Leader">
            ...
            <?php
                ...
                $select_team = "SELECT username as Team_Leader FROM employee WHERE  Status = 'Active' AND Role='Team Leader' ORDER BY username ASC";
