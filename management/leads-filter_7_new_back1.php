...
$Before7Datys =  date('Y.m.d',strtotime("-8 days"));
...
$sql = ("SELECT Id, Full_Name, Email, Mobile, Disposition, Source, UserName, Date, DateTime, TimeStamp, ModalFowllowUpDateTime FROM Assigned_Leads WHERE TimeStamp <= '".$Before7Datys."' && `username` ='".$username."' && `Disposition` ='".$disposition."' && `Source` ='".$source_name."' ORDER BY  `Assigned_Leads`.`TimeStamp` DESC  LIMIT 0, $limit");
...
echo '<td>Modal Fowllow Up Date Time</td>';
echo '</tr>';
