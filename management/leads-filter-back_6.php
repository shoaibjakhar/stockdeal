...
$sql = ("SELECT Id, Full_Name, Email, Mobile, Disposition, Source, UserName, Date, DateTime, TimeStamp, ModalFowllowUpDateTime FROM Assigned_Leads WHERE (DATE >= '".$StartDateUSA."') AND  (DATE<= '".$EndtDateUSA."') && `username` ='".$username."' && `Disposition` ='".$disposition."' && `Source` ='".$source_name."' ORDER BY  `Assigned_Leads`.`Id` DESC  LIMIT 0, $limit" );
