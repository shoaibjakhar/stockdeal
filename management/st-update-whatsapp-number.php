...
$connect = mysqli_connect('localhost','childshk_wp397','3P-S1p74[C','childshk_wp397');
if(!$connect)
{
...
//mysql_select_db('shareevx_wp510', $connect);
 ...
 $sql = "SELECT * FROM `wpx5_options` WHERE  option_id = '112853'";
...
if($_POST){
   ...
   $upd_qry = "UPDATE `wpx5_options` SET `option_value` = '".$new_content."' WHERE  option_id = '112853' ";
   mysqli_query($connect,$upd_qry);
  //header('location:');
  echo '<script>window.location.href="st-update-whatsapp-number.php?t='.time().'&updated=true"</script>';
