...
<?php
  ...
  */
  ...
  <head>
    ...
    <title>Inactive SR Team Lead Login Details</title>
  ...
  <body>
    ...
    <div class="main_container">
      ...
      <div class="breadcurms">
        ...
        <div class="pull-left">
          ...
          <?php if($_SESSION['Role'] == "Super Admin"){ ?>
            ...
            | <a href="inactive-sr-tl-login-details.php?filter=admin" class="btn btn-xs btn-primary">InActive SR Team Leader login details</a>
          <?php } ?>
        </div>
      ...
      <div class="containter" style="padding:20px 20px 0 20px;">
        ...
        if(isset($_GET['filter']) && $_GET['filter'] == 'admin'){
          $getQuery = "SELECT * from employee  where  Role='SR_TL' AND status='disabled' ORDER BY id ASC ";  
        }
    ...
    if(isset($_POST['submit_search']) && isset($_GET['filter']) && $_GET['filter'] == 'admin')
    {
      ...
      $sql = ( "SELECT * FROM employee  where status='disabled' AND Role='SR_TL' AND (username LIKE '%".$search_value."%' Or Team_Leader LIKE '%".$search_value."%') ORDER BY  `employee`.`Status`,`username` ");
    }
    else if(isset($_POST['submit_search']) && $_SESSION['Role'] != 'Team Leader')
    {
      ...
      $sql = ( "SELECT * FROM employee  where status='disabled' AND Role='SR_TL' ORDER BY  `employee`.`Status`,`username` ASC LIMIT ".$initial_page." ,".$limit."");
  ...
  <div class="pagination" style="padding-right: 20px;padding-top: 15px;padding-bottom: 15px;">
    ...
    for($page_number = 1; $page_number <= $total_pages; $page_number++) 
    {
     if($page_number == $selected_page_number){  
       ?>
       <?php   if(isset($_GET['filter']) && $_GET['filter'] == 'admin'){?>
        <a class="btn btn-success" href = "inactive-sr-tl-login-details.php?page=<?php echo $page_number;?>&filter=admin"><?php echo $page_number; ?></a>
      <?php }else{?>
       <a class="btn btn-success" href = "inactive-sr-tl-login-details.php?page=<?php echo $page_number;?>"><?php echo $page_number; ?></a>
     <?php }?>
   <?php }else{?>
     <?php   if(isset($_GET['filter']) && $_GET['filter'] == 'admin'){?>
      <a class="btn btn-primary" href = "inactive-sr-tl-login-details.php?page=<?php echo $page_number;?>&filter=admin"><?php echo $page_number; ?></a>
    <?php }else{?>
     <a class="btn btn-primary" href = "inactive-sr-tl-login-details.php?page=<?php echo $page_number;?>"><?php echo $page_number; ?></a>
   <?php }?>
 <?php }}  
 ?>
</div>
  ...
  this.setAttribute(
    ...
    .format( this.getAttribute("data-date-format") )
      ...
      <script>
        ...
        $(function(){
          $.get("suggestion-search.php",{ role:"SR_TL", status:"inActive" },function(response){
