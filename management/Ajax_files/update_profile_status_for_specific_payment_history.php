...
if($_POST){
    ...
    $upd = "update Customer_Payment_History set Approval_Status = 'Approved' where id = '".$_POST['row_id']."'";
    mysqli_query($connect,$upd);
    //die("here");
    ...
    $sel = "SELECT Full_Name,Email_ID,SUM(Paid_Amout) AS total_pay,PackageName,Activation_Date,Exp_Date,Paid_Amout FROM Customer_Payment_History WHERE Approval_Status = 'Approved' AND Costumer_ID = '".$_POST['CustomerId']."'";
...
$sql = ("SELECT  id, SUM(Paid_Amout) AS total_pay, SUM(Tax_Amount) AS total_tax,SUM(Company_Amount) AS Gross_Amount, Costumer_ID, DATE_FORMAT( SaleDate, '%d-%m-%Y' ) AS SaleDateIND, Full_Name, Email_ID, Mobile_No, Pan_Number, Approval_Status, PackageName, DATE_FORMAT( Activation_Date, '%d-%m-%Y' ) AS ActivationDate , DATE_FORMAT( Exp_Date, '%d-%m-%Y' ) AS ExpDate, Remark, Paid_Amout, Company_Amount, Tax_Amount, PaymentMode, Agent_1, Agent_1_Percentange, Agent_1_Shared_Amount,Agent_2, Agent_2_Percentange, Agent_2_Shared_Amount,Agent_3, Agent_3_Percentange, Agent_3_Shared_Amount, Date_of_Birth, KYC, Risk_Score, Risk_Level, DATE_FORMAT( DateTime, '%d-%m-%Y %h %i' ) AS DateTimeConvert FROM Customer_Payment_History where Approval_Status = 'Approved' AND Costumer_ID=".$_POST['CustomerId']." ORDER BY `Id` DESC LIMIT 50");
   ...
   $row = $result->fetch_assoc();
    ...
    $qury="SELECT * FROM Customer_Payment_History WHERE Approval_Status = 'Approved' AND Costumer_ID='".$_POST['CustomerId']."' ORDER BY `Id` DESC LIMIT 1";
...
$save=$pdf->Output('F',$fileName,true);
   ...
   // header('location:'.$_SERVER['HTTP_REFERER']);
    ...
    function sendEmail($to,$message,$id,$fileName)
    {
        ...
        // $mail->Password = "2T1zlj_14*KT2211";                  // SMTP password
        ...
        $upd = "UPDATE Customer_Payment_History SET Trading_Guidence_Mail = 'Sent' WHERE id = '".$_POST['row_id']."'";
         mysqli_query($connect,$upd);
