...
$sql = "SELECT Id, Full_Name, Email, Mobile, Segment, Disposition, Source,  State, UserName, Date, DateTime, TimeStamp, ModalFowllowUpDateTime FROM Assigned_Leads WHERE (Status IS NULL OR Status='Open') AND `Disposition` ='Fresh' ORDER BY  `Assigned_Leads`.`Id` DESC  LIMIT 0, $limit";
...
<select id='Agent' class='form-control AgentNames' name="AgentNames">
  ...
  <?php
   $sel = "SELECT * FROM `admin` WHERE Role = 'Team Leader' AND Status = 'Active'";
    $qry = mysql_query($sel);
