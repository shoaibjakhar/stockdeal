...
if($_SESSION['Role'] == 'SR_TL'){
  //die('here');
    $sel_data = ("SELECT Id, Full_Name, Email, Mobile, State, Source, Disposition,  UserName, Message, remoteAddress, Status, Investment, Segment,  DATE_FORMAT(LeadDateTime,  '%d/%m/%Y %h:%i') AS DateTimeINR FROM  `Assigned_Leads` WHERE UserName = '".$username."' AND Disposition = 'Fresh' ORDER BY  `id` DESC");
}
    ...
    $sel_data = ("SELECT Id, Full_Name, Email, Mobile, State, Source, Disposition,  UserName, Message, remoteAddress, Status, Investment, Segment,  DATE_FORMAT(LeadDateTime,  '%d/%m/%Y %h:%i') AS DateTimeINR FROM  `Assigned_Leads` WHERE (Status IS NULL OR Status='Open') AND Disposition = 'Fresh' ORDER BY  `id` DESC");
...
?>
<form name="bulk_action_form" action="assign-bulk-leads-back.php" method="post" onSubmit="return delete_confirm();"/>
    <table style="margin-bottom:10px;" border="0" class="">
  <tr>
   <!-- <td style="width: 100px; border-top:none;"> <input type="checkbox" name="select_all " id="select_all" value=""/> Select All</td> -->
    ...
    <td style="border-top:none;padding: 0;">
        <select class="form-control" name="AgentNames" id="AgentNames" style="width: 280px;"><?php 
    ...
    <td><input type="number" name="" value="0" class="form-control bulk_assignment_count" width="30%"></td>
    <td style="border-top:none;padding: 0;">
        <input type="submit" style="display: none" class="btn btn-primary bulk_delete_submit" name="bulk_delete_submit" value="SUBMIT"/>
    </td>  
  </tr>
</table>

  
    <table id="" class="table table-bordered add_bulk_assignment_count" cellspacing="0" width="100%">
        <thead>
        <tr>
            <th></th>        
        ...
        if(!empty($leads)){
         ...
         ?>
               ...
               <td><?php echo $row['Investment']; ?></td>
                    ...
                    <td><?php echo isset($row['SubSource']) ? $row['SubSource'] :''; ?></td>
...
<script>
    $(document).ready(()=>{
        ...
        $(".bulk_assignment_count").keyup(()=>{
            var j = 0;
            var bulk_assignment_count = $(".bulk_assignment_count").val();
            ...
            $("input.checkbox").each(function(e){
                ...
                if(j==bulk_assignment_count){ 
                    return false;
                }
                
                $("input.checkbox")[e].checked = true;
                
                j++;
                
            });
            
            $(".CountSelected").text(bulk_assignment_count);
