...
<?php include('connection/dbconnection_crm.php')?>
 ...
 else if($_SESSION['Role'] == 'Team Leader' &&  $Source =='ALL') {
     $sql = "SELECT Id, Full_Name, Email, Mobile, Segment, Disposition, Source,  State, UserName, Date, DateTime, TimeStamp, ModalFowllowUpDateTime FROM Assigned_Leads WHERE `username` ='$username' && `Disposition` ='Fresh' ORDER BY  `Assigned_Leads`.`Id` DESC  LIMIT 0, $limit";  
 }
 
 
 else if($_SESSION['Role'] == 'Team Leader') {
     $sql = "SELECT Id, Full_Name, Email, Mobile, Segment, Disposition, Source,  State, UserName, Date, DateTime, TimeStamp, ModalFowllowUpDateTime FROM Assigned_Leads WHERE `username` ='$username' && `Disposition` ='Fresh' && `Source` ='".$Source."' ORDER BY  `Assigned_Leads`.`Id` DESC  LIMIT 0, $limit";  
 }


 
$result = mysqli_query($connect, $sql) or die($sql."<br/><br/>".mysqli_error());
...
<select id='Agent' class='form-control AgentNames' name="AgentNames">
  ...
  <?php
  $sel = "SELECT username from employee where Team_Leader = '$username' and Status='Active' order by username ASC";
