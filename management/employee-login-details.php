...
if(isset($_POST['submit_search']) && isset($_GET['filter']) && $_GET['filter'] == 'admin')
{
  ...
  $sql = ( "SELECT * FROM employee  where status='Active' AND Role='Team Leader' AND (username LIKE '%".$search_value."%' Or Team_Leader LIKE '%".$search_value."%') ORDER BY  `employee`.`Status`,`username` ");
}
else if(isset($_POST['submit_search']) && $_SESSION['Role'] != 'Team Leader')
{

  $search_value = $_POST['search'];
  $sql = ( "SELECT * FROM employee  where status='Active' AND Role='Agent' AND (username LIKE '%".$search_value."%' Or Team_Leader LIKE '%".$search_value."%') ORDER BY  `employee`.`Status`,`username` ");
}
else if(isset($_GET['filter']) && $_GET['filter'] == 'admin'){

  $sql = ( "SELECT * FROM employee  where status='Active' AND Role='Team Leader' ORDER BY  `employee`.`Status`,`username` ASC LIMIT ".$initial_page." ,".$limit."");
