    ...
    <div class="panel panel-default" style="">
      <div class="panel-heading font-size16">
       ...
       if($monthName == 'Jan'){
        ...
        echo "team_lead_attendance.php?previous_month=".$previous_month; 
      }
  ...
  <table id="Admin_Attendance" class="display nowrap table table-bordered table-hover-attendance" style="width:100%">
    ...
    <tbody>
      ...
      if($_SESSION['Role'] == 'Team Leader'){
       ...
       $qry_agents = "SELECT Id, username, Bank_Details,Account_NO,IFSC_Code, salery, Role, Date_of_Join from employee where Status = 'Active' AND username !='Akshay Shetty' AND username !='Select' AND Role='Team Leader'";
     }
     ...
     while($get_agents = $get_qry->fetch_assoc()){
      ...
      <tr>
        ...
        <td><?php echo $get_agents['Account_NO'] ?></td>
...
$TL_agents = "SELECT username from employee where Status = 'Active' AND username !='Akshay Shetty' AND Team_Leader='".$get_agents['username']."' AND Role='Agent'";
                  //$TL_agents = "SELECT username from employee where Status = 'Active' AND username !='Akshay Shetty' AND Team_Leader='Mani Devendra' AND Role='Agent'";


$tl_get_qry = mysqli_query($connect,$TL_agents);
$tl_agent_sale=0;
$sale = 0;

while($get_agents_name = $tl_get_qry->fetch_assoc())
{

  $tl_agent_sale = getShared($get_agents_name['username']);
                      //echo $i." ".$get_agents_name['username']."=".$tl_agent_sale."=<br>";
  $sale =  $sale + $tl_agent_sale;
