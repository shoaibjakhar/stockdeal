...
//echo ($StartDateUSA . $Packages  . $EndtDateUSA . $Packageresults);
...
$sql = ("SELECT * FROM stock_tips WHERE Date >= '".$StartDateUSA."' AND Date <= '".$EndtDateUSA."' && `Sagment` ='".$Packages."' && `Result` ='".$Packageresults."' limit 30");
...
echo('<tr>');
  ...
  echo('<td class="btn '.$row['Result'].'" style="margin-left:10px;margin-right:10px">'.$row['Result'].'</td>');
