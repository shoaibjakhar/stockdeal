...
$qry_show_hide = mysql_query($sel_sho_hide);
$fetch_show_hide = mysql_fetch_assoc($qry_show_hide);
  ...
  <div class="containter" style="padding:20px 20px 0 20px;">
   ...
   // while ( $row = mysql_fetch_array( $result ) ) {
		...
		if($show_hide['Date_of_Birth']){
		    echo('<div class="col-sm-3"><label>Date_Of_Birth</label><input type="date" readonly name="Date_of_Birth" value="' . date('Y-m-d',strtotime($row[ 'Date_of_Birth' ])) . '" class="form-control"/></div>');
		}
		if($show_hide['KYC']){
    		$sel_kyc = "SELECT KYC FROM Options WHERE KYC IS NOT NULL";
    		$qry_kyc = mysql_query($sel_kyc);
    		echo ('<div class="col-sm-3"><label>KYC</label><select disabled name="KYC" required class="form-control">');
    		
    		while($Kyc_details = mysql_fetch_assoc($qry_kyc)){
		...
		if($show_hide['PanNumber']){
		    echo('<div class="col-sm-3"><label>Pan_Number</label><input readonly type="text" name="Pan_Number" value="' .strtoupper($row['Pan_Number']) . '" class="form-control"/></div>');
		}
		if($show_hide['Risk_Score']){
		    echo('<div class="col-sm-3"><label>Risk_Score</label><input readonly type="text" name="Risk_Score" value="' . ucfirst($row[ 'Risk_Score' ]) . '" class="form-control"/></div>');
		}
		echo('<div class="col-sm-3"><label>Mobile No</label><input type="text" readonly name="Mobile_No" value="' . $row[ 'Mobile_No' ] . '" class="form-control"/></div>');
		echo('<div class="col-sm-3"><label>Full_Name</label><input type="text" readonly name="Full_Name" value="' . ucfirst($row[ 'Full_Name' ]) . '" class="form-control"/></div>');
		
		echo('<div class="col-sm-3"><label>Email ID</label><input type="text" readonly name="Email_ID" value="' . strtolower($row[ 'Email_ID' ]) . '" class="form-control"/></div>');
...
// 							$getd_qry = mysql_query($get_risk_qry);
...
// 					echo $datas_segments;
            ...
            $sel_segment = "SELECT Segment, Segment_Amount FROM Options WHERE Segment IS NOT NULL";
                $qry_segment = mysql_query($sel_segment);
                while($fetch_segment = mysql_fetch_assoc($qry_segment)){
		...
		<div class="col-sm-3">
			...
			<input type="text" readonly class="form-control" readonly value="" id="PackagePrice" name="PackagePrice" />
		</div>
		<div class="col-sm-3">
			<label for="">Sale Date*</label>
			<input type="text" readonly value="<?php echo date('d-m-Y',strtotime($row['SaleDate'])); ?>" id="SaleDate" name="SaleDate"  class="form-control" placeholder="Sale Date"  autocomplete="off" required/>
	     ...
	     <div class="col-sm-3">
			...
			<input type="text" readonly  value="<?php echo date('d-m-Y',strtotime($row['Activation_Date'])) ?>" id="Activation_Date"  onchange="CalculateDays()" name="Activation_Date" class="form-control" placeholder="Activation Date"  autocomplete="off" required/>
		...
		<div class="col-sm-3">
			...
			<input type="text" readonly value="<?php echo date('d-m-Y',strtotime($row['Exp_Date'])) ?>" id="Exp_Date" name="Exp_Date" onchange="CalculateDays();$('#result_exp_date_error').text('')"  class="form-control" placeholder="Expired Date"  autocomplete="off" required/>
        ...
        <div class="col-sm-3">
			<label for="">Number of days*</label>
				<input type="text" value="" readonly id="Number_of_Days" readonly name="Number_of_Days" class="form-control inherit" placeholder="Number of days" required/>
		</div>
	    <div class="col-sm-3">
			<label for="">Payment Mode*</label>
			<select class="form-control" id="PaymentMode" name="PaymentMode" required disabled>
            <?php
                $sql_payment_mode = ("SELECT payment_mode FROM `Options` WHERE payment_mode IS NOT NULL");
                // echo('<option value="" >Payment Mode</option>');
                $result_payment_mode = mysql_query($sql_payment_mode);
		...
		<div class="col-sm-3">
			...
			<input type="text" value="<?php echo $row['Remark']; ?>" id="Remark" readonly name="Remark" class="form-control inherit" placeholder="Remark" required/>
	    </div>
		<div class="col-sm-3">
		    <label for="">PPE Credits</label>
			<input type="text" value="<?php echo $row['PPI_Credits']; ?>" readonly class="form-control" name="PPI_Credits" placeholder="PPE Credit">
		</div>
...
$qry_amount_paid = mysql_query($sel_amount_paid);
$fetch_amount_paid = mysql_fetch_assoc($qry_amount_paid);
