...
<form class="form-inline">
  ...
  <div class="form-group">
    ...
    <option value="">Select</option>
			<?php
			$get_risk_qry = "SELECT Stock_Tips_Status FROM `Options` where Stock_Tips_Status IS NOT NULL";
			$getd_qry = mysqli_query($connect, $get_risk_qry );
			while ( $get_options = mysqli_fetch_assoc( $getd_qry ) ) {
				?>
			<option value="<?php echo $get_options['Stock_Tips_Status']; ?>">
				<?php echo $get_options['Stock_Tips_Status'] ?>
			</option>
			<?php
			//	echo '<option value="'.$get_options['Package'].'">.$get_options["Package"].</option> ';
			}
