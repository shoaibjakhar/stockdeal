...
<div class="breadcurms">
 <div class="pull-left">
<a href="dashboard.php">Dashbord</a> | <a href="customer-profile-new-3-month.php">Customer details</a> | <a href="customer-profile-new-3-month-details.php" class="btn btn-xs btn-primary">All Payment History</a>
 </div>
...
$sql = ("SELECT Costumer_ID, DATE_FORMAT( SaleDate,  '%d-%m-%Y' ) AS SaleDateIND, Full_Name,  Email_ID, Approval_Status, Mobile_No, Pan_Number, PackageName,  DATE_FORMAT( Activation_Date,  '%d-%m-%Y' ) AS ActivationDate ,
...
Date_of_Birth, KYC, Risk_Score, Risk_Level, DATE_FORMAT( DateTime,  '%d-%m-%Y %h %i' ) AS DateTimeConvert  FROM Customer_Payment_History where (SaleDate>= DATE_FORMAT(CURDATE(),'%Y-%m-01') - INTERVAL 2 MONTH) AND (Agent_1 = '".$username."' OR Agent_2 = '".$username."' OR Agent_3 = '".$username."') ORDER BY  `DateTime` DESC ");
...
{
echo('<tr>');
	 echo('<td><a heft="#" class="btn btn-danger">'.$row['Approval_Status'].'</a></td>');
